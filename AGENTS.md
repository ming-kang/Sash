...
- Keep each responsibility in one place and reuse existing state. Add machinery only for a concrete need. Ask before removing deliberate functionality.
...
- Read local records leniently: accept unknown fields and treat unreadable records as absent or stale.
- Validate build output during the build.
...
- **Downloads:** origins must be allowlisted in `github.ts`; verify archives against the release API's SHA-256 while streaming. Extraction rejects path traversal and enforces a size cap.
...
- **Self-upgrade:** run npm's global install for one exact version while Sash keeps serving the proxy; only then restart Sash (`--no-restart` skips restarting). npm owns package integrity.
...
- Describe states the user can act on and keep internal bookkeeping out of prose. PIDs appear only where useful for troubleshooting and always in `--json`.
...
- Test current behavior. After code changes: `npm run typecheck`, `npm run lint`, then affected tests once. Tests live beside modules as `*.test.ts`; `npm test -- <filename-or-path-fragment>` filters them. Do not repeat passing checks without a new reason.
