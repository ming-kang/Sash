...
`sash upgrade [version]` updates the Sash package, daemon and bundled dashboard in one operation. `sash update [tag]` updates Core. This contract defines the complete upgrade experience.
...
- Let npm install the exact candidate version and dependencies in an isolated staging prefix on the installation's filesystem. Use the official registry and isolated npm configuration, scrub child credentials, and let npm own archive integrity. Do not download/hash the same package separately or run a second dependency-tree check after installation.
- Run one candidate probe covering runtime imports and dashboard assets while the original runtime remains available. Prepare recovery material before downtime. Activation must not repeat the probe or need another network download.

## Durable installation transaction

The updater runs from a standalone Node helper outside the package directory. The helper must continue to work while the installed package is absent, replaced or invalid. Bundle its dependencies when building Sash; CI's package smoke exercises its independent execution. Runtime upgrades do not repeat its self-test or hash it at each stage.

Stage and activate the npm-prepared package and its bin shims as fixed filesystem roles. Retain the original package and shims until every restored runtime passes its readiness check. Record decisions through atomic, bounded journals. Directory device/inode identities distinguish package slots across renames without reading every file; fixed path boundaries and regular-file checks prevent writes through unrelated links. Keep npm's installation layout and invalidate its disposable installation cache through the owned package directory's modification time.
...
Transaction stages are preparation, instance reservation, stopping, activation, runtime restoration, commit and cleanup. Recovery inspects the journal and directory identities; it resumes committed cleanup or restores the prior installation and runtime. Cancellation before activation releases reservations; cancellation or failure afterward rolls back. Unverifiable process or directory ownership preserves recovery material. Once instances and command shims are settled, release startup admission before deleting old packages. A leftover temporary directory produces a warning without undoing a successful upgrade.
...
Restart through the prepared package and supported Node executable, then check the daemon's startup version, instance identity and restored runtime once before committing. Recovery after a later interruption obtains fresh observations. Preserve the data directory and working login startup registration. Failed installation or readiness restores the previous package and the same runtime snapshot without depending on network access.
...
Verify recovery through the installed bin shim while the active package is absent. Check restored runtime behavior, configuration and browser access in isolated integration tests. CI builds and packs once; each supported platform installs the same artifact and exercises its command entries and independent worker. Publication reuses the successful CI artifact without repeating acceptance. Exercise Windows paths containing spaces/Unicode and ensure helpers neither expose credentials nor terminate unrelated processes.
